 ...
 */
public class QBEA1 extends LearningAlgorithm {
    ...
    public QBEA1(Environment environment, int seed) {
...
//            System.out.println(" state:" + state.center + " next state:" + nextState.center);
    ...
    private void useEAtoUpdateQValues(int timeStep, State state) {
        ...
        for (Action evalAction : environment.getActions()) {
            State probableState = (currentState.center == 5 ? new State(-5) : new State(5)); //estimateNextState(state, evalAction);
            int reward = environment.getReward(evalAction);
            updatePolicy(currentState, probableState, evalAction, reward, timeStep);
        }
    }
